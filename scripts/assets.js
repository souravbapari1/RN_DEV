...
function walkDir(currentPath, relativePath = '') {
    ...
    for (const entry of entries) {
        ...
        const relEntryPath = path.join(relativePath, entry).replace(/\\/g, '/');

        // ❌ Skip 'bootsplash' folder
        if (stat.isDirectory() && relEntryPath.startsWith('bootsplash')) {
            continue;
        }
        ...
        if (stat.isDirectory()) {
            ...
            const key = path
                .join(relativePath, path.basename(entry, path.extname(entry)))
                .replace(/\\/g, '/')
...
// Format output
const content = `// Auto-generated by scripts/assets.js
...
console.log('✅ assets.ts generated successfully!');
